...
2. **📈 銘柄ニュース・材料（Stock Intelligence）**
   - `asset-aggregator` の全30保有銘柄（三菱重工、フジクラ、NXHD、日立、三井住友、JT等）および購入検討銘柄（ウォッチリスト）の最新動向をAIが要約
   - 直近ニュース、決算・適時開示、好材料・リスク要因、AI投資インサイト、材料バッジをカード形式で表示
   - タブレット画面上から新しい購入検討銘柄の追加・削除管理も可能
3. **💡 思考整理・ブレスト（Brainstorm & Wall-hit）**
   - アイデアや課題を入力すると、Geminiが即座に多角的視点から壁打ち
   - 「共感・アイデア拡張」「批判的検証（悪魔の代弁者）」「ネクストアクション3選」の3枚のカードで出力
4. **📝 タスク・アドバイザー（Daily Task Advisor）**
